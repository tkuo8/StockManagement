...
#### ユーザーストーリー 2: 保有株の一覧を表示する画面(ダッシュボード)を作成する
...
- **タスク 2.3**: 通知の手段として Web 通知を実装する。
  ...
  - **詳細**: 株価が目標額に達した場合、ユーザーに通知を送る仕組みを実装。バックエンドで株価の監視と通知機能を追加する。ブラウザの通知 API を利用して、アラートをユーザーに表示する。
...
- **タスク 2.5**: 通知履歴を管理する機能を作成する。
  - **詳細**: 過去のアラート通知履歴を保存し、ユーザーが確認できるようにする。
  - **完了基準**: 通知履歴が画面に表示される。

#### ユーザーストーリー3: 目標達成度の管理機能を実装
- **タスク2.6**: 各銘柄ごとの目標額に対する達成度を表示する機能を作成する。
  - **詳細**: ユーザーが設定した目標額と、現在の評価額に基づいて目標達成度を計算し、画面に表示する。
  - **完了基準**: 各銘柄に対する目標達成度が表示される。

- **タスク2.7**: 目標達成度をグラフで視覚的に表示する機能を実装する（Chart.js使用）。
...
#### ユーザーストーリー 2: ダッシュボードを改良する

- **タスク 3.3**: 投資進捗を視覚化するダッシュボードを改良する。
  - **詳細**: ユーザーの総資産や目標達成度を一目で確認できるようにダッシュボードを改良する。
